 ...
 * method `toLatex` can be used to generate LaTeX code to draw a histogram, its
 * polygonal interpolation, or both in a document.
 * 
...
public class ScaledHistogram {
    ...
    * uses the same interval @f$[a,b]@f$ and the same bins, and keeps the bin
    * counts as the rectangle heights.
    */
   public void init(TallyHistogram hist) {
      ...
      height = new double[numBins];
      this.integral = hist.numberObs() * m_h; // Do we have to use obs inside [a,b] instead ? 
   ...
   public void init(TallyHistogram hist, double integral) {
      ...
      int count[] = hist.getCounters();
      double scaleFactor = integral / (hist.numberObs() * m_h); // Do we have to use obs inside [a,b] instead ?
      for (int i = 0; i < numBins; i++)
   ...
   /**
    * Sets the LaTeX options for the axis generated by {@code toLatex}. When these
    * options are nonempty, they replace the default axis options generated by
    * {@code toLatex}. The string must contain comma-separated options without
    * brackets. Passing {@code null} clears the options and restores the default
    * axis options.
    *
   ...
   public ScaledHistogram averageShiftedHistogram1(int r) {
      ...
      S2[0] = 0.0;
      heightNew[0] = r * S1[0];
      for (int ell = 1; ell <= Math.min(r, numBins-1); ell++) {
         S2[0] += height[ell];
